...
public class ConsumerThread implements Consumer {
    ...
    public ConsumerThread(Properties properties, Database database) {
        ...
        this.consumer = new KafkaConsumer<>(properties, new StringDeserializer(), new JsonDeserializer<>(student.class));
    ...
    public void run() {
        try {
            while (true) {
                ConsumerRecords<String, student> records = this.consumer.poll(Duration.ofMillis(1000));
